...
- **Smoke flows through open doors**: an open exterior door flushes a smoky room fast and *visibly* (about 40% per second — a full room clears in ~5 seconds), and open interior doors spill smoke into the next room so it spreads toward an exit instead of sitting still. The ventilation guarantee changed shape to match: burners and inflows can never push a properly ventilated room past a light haze, but thick pre-existing smoke now drains through the outlets on screen instead of vanishing to the cap in one tick.
