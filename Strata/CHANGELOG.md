...
### Added
- **Natural gases** and **Pollutant gases** mod settings (both default ON) — split atmospheric breathing mix (N₂, O₂, Ar, ambient CO₂, hypoxia, plant exchange) from harmful extras (smoke, methane, deep gas, spores, steam, black damp). Each toggle clears its channels, skips sim emit/drain/harm with early-outs, and hides matching overlay tints. Smoke inhalation severity nests under Pollutant gases; settings migration v2 maps old Smoke simulation / O₂–CO₂ toggles. Replaces separate Smoke simulation and O₂ / CO₂ simulation checkboxes in the UI.
