...
- **Strata** — gravship stairwells/elevators no longer vanish on launch (`gravship-stairs-launch-v1`). Odyssey pack `DeSpawn` was swallowed by portal immunity, then land skipped `MakeThing` restore. → [Strata/CHANGELOG.md](Strata/CHANGELOG.md)
- **Repo** — local Windows deploys target the Dev RimWorld Mods folder only (`E:\SteamLibrary\steamapps\common\RimWorld - Dev\Mods`); `scripts/deploy-mods.ps1` refuses the gaming install. Cloud agents still link `/opt/rimworld/RimWorld/Mods`.
