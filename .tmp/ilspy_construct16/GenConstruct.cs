...
public static class GenConstruct
{
	private readonly struct ResourcesKey : IEquatable<ResourcesKey>
	{
		...
		public ResourcesKey(Pawn pawn, Thing thing, IConstructible constructible, int neededHash, bool forced)
		{
			mapId = pawn.Map.uniqueID;
			pawnId = pawn.thingIDNumber;
			entityToBuild = thing.def.entityDefToBuild;
			stuff = constructible.EntityToBuildStuff();
			this.neededHash = neededHash;
			this.forced = forced;
		}

		public override bool Equals(object obj)
		{
			if (!(obj is ResourcesKey resourcesKey))
			{
				return false;
			}
			if (mapId == resourcesKey.mapId && pawnId == resourcesKey.pawnId && entityToBuild == resourcesKey.entityToBuild && stuff == resourcesKey.stuff && neededHash == resourcesKey.neededHash)
			{
				return forced == resourcesKey.forced;
			}
			return false;
		}
		...
		{
			return Gen.HashCombine(Gen.HashCombine(Gen.HashCombine(Gen.HashCombine(Gen.HashCombine(Gen.HashCombine(0, mapId), pawnId), entityToBuild), stuff), neededHash), forced);
		}

		public bool Equals(ResourcesKey other)
		{
			if (mapId == other.mapId && pawnId == other.pawnId && object.Equals(entityToBuild, other.entityToBuild) && object.Equals(stuff, other.stuff) && neededHash == other.neededHash)
			{
	...
	private static readonly Dictionary<ResourcesKey, bool> canGetResourcesCache = new Dictionary<ResourcesKey, bool>();

	private static int canGetResourcesCacheTick = -1;
	...
	public static void Reset()
	{
		...
		canGetResourcesCache.Clear();
		canGetResourcesCacheTick = -1;
	}

	public static bool CanGetResources(Thing thing, Pawn pawn)
	{
		return CanGetResources_NewTemp(thing, pawn);
	}

	public static bool CanGetResources_NewTemp(Thing thing, Pawn pawn, bool forced = false)
	{
		...
		if (thing.Map == null || pawn.Map == null || thing.Map != pawn.Map)
		{
			return false;
		}
		List<ThingDefCountClass> list = constructible.TotalMaterialCost();
		if (list.NullOrEmpty())
		{
			return true;
		}
		int num = 0;
		bool flag = false;
		foreach (ThingDefCountClass item in list)
		{
			int num2 = NeededNow_NewTemp(constructible, item.thingDef, pawn, forced);
			if (num2 > 0)
			{
				flag = true;
				num = Gen.HashCombine(num, item.thingDef);
				num = Gen.HashCombine(num, num2);
			}
		}
		if (!flag)
		{
		...
		if (canGetResourcesCacheTick != ticksGame)
		{
			canGetResourcesCache.Clear();
			canGetResourcesCacheTick = ticksGame;
		}
		ResourcesKey key = new ResourcesKey(pawn, thing, constructible, num, forced);
		if (canGetResourcesCache.TryGetValue(key, out var value))
		{
			return value;
		}
		value = true;
		foreach (ThingDefCountClass item2 in list)
		{
			int num3 = NeededNow_NewTemp(constructible, item2.thingDef, pawn, forced);
			if (num3 > 0 && !pawn.Map.itemAvailability.ThingsAvailableAnywhere(item2.thingDef, num3, pawn))
			{
				value = false;
				break;
			}
		}
		canGetResourcesCache.Add(key, value);
		return value;
	}

	private static int NeededNow(IConstructible constructible, ThingDef need, Pawn pawn)
	{
		return NeededNow_NewTemp(constructible, need, pawn);
	}

	private static int NeededNow_NewTemp(IConstructible constructible, ThingDef need, Pawn pawn, bool forced = false)
	{
		if (!forced && constructible is IHaulEnroute enroute)
		{
			return enroute.GetSpaceRemainingWithEnroute(need, pawn);
		}
		return constructible.ThingCountNeeded(need);
	}
	...
	{
		return CanPlaceBlueprintAt_NewTemp(entDef, center, rot, map, godMode, thingToIgnore, thing, stuffDef, ignoreEdgeArea, ignoreInteractionSpots, ignoreClearableFreeBuildings);
	}

	public static AcceptanceReport CanPlaceBlueprintAt_NewTemp(BuildableDef entDef, IntVec3 center, Rot4 rot, Map map, bool godMode = false, Thing thingToIgnore = null, Thing thing = null, ThingDef stuffDef = null, bool ignoreEdgeArea = false, bool ignoreInteractionSpots = false, bool ignoreClearableFreeBuildings = false, Predicate<Thing> skipBlockingThing = null)
	{
		...
		for (int i = 0; i < thingList.Count; i++)
		{
			...
			if (thing2 == thingToIgnore || (skipBlockingThing != null && skipBlockingThing(thing2)) || thing2.Position != center || thing2.Rotation != rot)
			{
				continue;
			}
		...
		if (!godMode)
		{
			foreach (IntVec3 item2 in cellRect)
			{
				...
				for (int k = 0; k < thingList.Count; k++)
				{
					...
					if (thing3 == thingToIgnore || (skipBlockingThing != null && skipBlockingThing(thing3)) || (thing3 is Building building && building.IsClearableFreeBuilding && ignoreClearableFreeBuildings))
					{
						continue;
					}
