...
public class JobDriver_ABTakePawnAcrossLevels : JobDriver
{
	...
	private Pawn Victim
	{
		get
		{
			...
			return (Pawn)(object)((thing is Pawn) ? thing : null);
		}
	}
	...
	{
		ToilFailConditions.FailOnDestroyedOrNull<JobDriver_ABTakePawnAcrossLevels>(this, (TargetIndex)1);
		ToilFailConditions.FailOnDespawnedOrNull<JobDriver_ABTakePawnAcrossLevels>(this, (TargetIndex)2);
		ToilFailConditions.FailOn<JobDriver_ABTakePawnAcrossLevels>(this, (Func<bool>)(() => Stairs == null || !Stairs.HasAnyLink));
		ToilFailConditions.FailOn<JobDriver_ABTakePawnAcrossLevels>(this, (Func<bool>)(() => Victim == null || Victim.Dead || ((base.job.def == ABDefOf.AB_TakePrisonerAcrossLevels) ? (!Victim.IsPrisonerOfColony) : (!Victim.Downed))));
		yield return ToilFailConditions.FailOnSomeonePhysicallyInteracting<Toil>(Toils_Goto.GotoThing((TargetIndex)1, (PathEndMode)1, false), (TargetIndex)1);
		...
		transfer.initAction = delegate
		{
			...
			if (base.job.def == ABDefOf.AB_CaptureAcrossLevels)
			{
				intent = CarriedIntent.Capture;
			}
			else if (base.job.def == ABDefOf.AB_TakePrisonerAcrossLevels)
			{
				intent = CarriedIntent.Imprison;
			}
			StairTransfer.Transfer(base.pawn, Stairs, intent, explicitDest);
		};
