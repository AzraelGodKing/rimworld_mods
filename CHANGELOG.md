...
- **Strata** — foreign portal exclusion (`foreign-portal-exclusion-v1`): relays/alerts no longer treat other mods' portal maps (Deep And Deeper caves, Anomaly undercaves) as base floors; opt-in setting restores it. → [Strata/CHANGELOG.md](Strata/CHANGELOG.md)
- **Ideas** — Homesteader next-content pool (Steam QoL + pantry/yard/farmstand) and Deep Colony Batch C (post-2.0, 20 ideas). → [docs/ideas/homesteader-updates.md](docs/ideas/homesteader-updates.md) · [docs/ideas/deep-colony-batch-c.md](docs/ideas/deep-colony-batch-c.md)
- **Homesteader** — optional unique texture pack behind a settings toggle (off by default); original sprites kept. Graphic_Multi `_*_{north,south,east,west}` files are one building from four angles. Diggo stays on brought HippoDogPlushie art; 27 statue stays on brought 27_2.0. Refresh outliers that did not read as RimWorld sprites were redrawn against actual Core furniture/item/plant sprites. Refresh wood-burning generator and wood stove redone as compact Core-style 1x1s; battery bank as a 2x2 industrial rack; compact battery as a chunky 1x1 cell; beehive as a wooden hive box; beeswax candle as an outlined fat candle; brewing bench four facings redone as one station; butter churn as a compact dash churn; canning kitchen as four cardinals of one 2x1 water-bath station; checkers table as a wooden carved-board 1x1; cheese press as a low table+hoop, cider press as a slatted barrel; curing rack four facings redone as one mixed-charcuterie rack; drying rack four facings redone as one 3x1 slatted table; grain mill as a compact hand mill; harvest maypole ribbon gaps are true alpha; hayloft redesigned as one open elevated loft from four angles; icehouse redesigned as four cardinals of one 2x2 ice cellar; compost heap redone as a 2x2 open-front wooden slat bay. → [Homesteader/CHANGELOG.md](Homesteader/CHANGELOG.md)
...
- **Docs Workshop stats** — hub/mod subscriber counts fetch live in-browser from Steam (no hourly GitHub Action). Roster at `docs/data/workshop-mods.json`; `stats-cache.json` kept only as offline fallback. "Refresh now" button on the hub clears the 1-hour browser cache and refetches live. → [docs/scripts/stats-display.js](docs/scripts/stats-display.js)
