...
Player-facing version **1.1.0** (`About.xml` `modVersion`). Startup writes `[Stormproof] v1.1.0 loaded from ...` with stamp `azr-49-58-77-78-79-80-v1`.
...
### Fixed
- **Guarded Harmony** — each patch class is applied on its own; one missing target logs and skips instead of aborting the rest of Stormproof.
