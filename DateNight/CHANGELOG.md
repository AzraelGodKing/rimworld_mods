...
### Fixed
- **Schedule Everything overlap** (`azr-14-schedule-combo-v3`) — Date is one extra Schedule cell immediately after the last extra button (Clean / Workout / …), drawn last so it is not inside another dropdown. Face is Date; Lovin is in the ▾ menu. The cell is clamped so it cannot cover Manage areas.
