...
- **Homesteader** — optional unique texture pack behind a settings toggle (off by default); original sprites kept. Graphic_Multi `_*_{north,south,east,west}` files are one building from four angles. Diggo stays on brought HippoDogPlushie art; 27 statue stays on brought 27_2.0. Refresh outliers that did not read as RimWorld sprites were redrawn against actual Core furniture/item/plant sprites. Refresh wood-burning generator and wood stove redone as compact Core-style 1x1s; battery bank as a 2x2 industrial rack; beehive as a wooden hive box; beeswax candle as an outlined fat candle; brewing bench four facings redone as one station. → [Homesteader/CHANGELOG.md](Homesteader/CHANGELOG.md)
