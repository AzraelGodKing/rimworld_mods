...
| --- | --- |
| **Levels** | **Down:** excavated stairwell → solid-rock Level −N the size of the map above. Dug cells clear fog; rock wall faces that border open stay visible; deep undug rock stays fogged. **Up:** tower stairwell / tower elevator → outdoor Level +N roof deck (build only where the floor below is roofed, plus the shaft plaza; open sky elsewhere). A second shaft on the same floor joins the SAME linked level. |
| **Fluidity** | Colonists relay themselves between floors for work, food, rest, medical, joy, hauling, construction materials, and rituals. Rest follows the assigned bed across floors (no wrong-floor bunk nap on the way). Work relay covers growing, cleaning, firefighting, hunting, research, warden, handling, and bills — mods can register extra probes. You designate; they commute. |
| **Purpose tags** | Tag a floor Farm, Freezer, Workshop, Barracks, Quarry, Storage, Hospital, or none (rename dialog or Levels tab). Matching tags tilt haul, rest, food, and work relays; untagged floors keep BFS order. A wrong tag never blocks a job. |
...
| **UI** | Levels tab (resizable) with jump/rename/purpose tag (Level +N / surface / Level −N), Page Up/Down level hotkeys (rebindable), mod settings for every major system, placement guides for vents and ducts. |
