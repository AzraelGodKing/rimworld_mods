...
- **Repo** — Removed decompiled RimWorld scratch under `Strata/Tools/`. Download zips no longer committed; CI builds all four mods (incl. Nemesis), publishes zips to the rolling `latest` GitHub Release, and PRs get compile-only checks. Root PowerShell utilities moved to `scripts/`.
