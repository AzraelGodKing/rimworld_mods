...
- **Construction demand pull**: a level whose blueprints and frames are short of materials now pulls them from other levels automatically — no stockpile setup needed. Haulers ship matching materials (loose or stored, any priority) through the stairs; once on the right level, vanilla construction delivery takes over. A level's own shortage always keeps first claim on its materials, shipments only go where a construction site is actually walkable from the landing, and the shortage ledger refreshes on a slow cadence so over-shipping self-corrects.
