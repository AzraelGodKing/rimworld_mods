...
- **Homesteader** — optional unique texture pack behind a settings toggle (off by default); original sprites kept. Graphic_Multi `_*_{north,south,east,west}` files are one building from four angles. Diggo stays on brought HippoDogPlushie art; 27 statue stays on brought 27_2.0. Refresh outliers that did not read as RimWorld sprites were redrawn against actual Core furniture/item/plant sprites. Refresh wood-burning generator redone as a compact Core-style 1x1; wood stove refresh redone the same way. → [Homesteader/CHANGELOG.md](Homesteader/CHANGELOG.md)
